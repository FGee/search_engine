...
void WebPage::webPageInit(const string xmlStr, CppJiebaSplitTool* pCppJieba)
{
    docToWeb(xmlStr);
    cutWord(pCppJieba);
    produceSimHash();
...
void WebPage::webPageInitNoXML(const string queryText, CppJiebaSplitTool* pCppJieba)
{
    _content = queryText;
    cutWord(pCppJieba);
}
...
void WebPage::cutWord(CppJiebaSplitTool* pCppJieba)
{
    istringstream iss(_content);

    string line;
    while (getline(iss, line)) {
        vector<string> wordsVec = pCppJieba->cut(line);
...
string WebPage::productSummary(const string& word)
{
    string::size_type pos = _content.find(word);
    return _content.substr(pos, 300);
}
